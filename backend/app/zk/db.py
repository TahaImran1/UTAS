...
PULL_HISTORY_FILE = os.path.join(APP_DATA_DIR, "pull_history.json")

def add_pull_history_entry(machine_ref: str, company: str, profile: str, count: int, status: str):
    """Appends an execution record to the pull history JSON file."""
    import datetime
    
    # Do not clutter history with 0-count success records from automatic syncs
    ...
    with pull_history_lock:
        ...
        if os.path.exists(PULL_HISTORY_FILE):
            try:
                ...
                history = []
        history.insert(0, entry) # Prepend so newest is first
        
        # Keep only the last 5000 records to prevent file growing indefinitely
        ...
        try:
            ...
            logging.error(f"Failed to write to pull_history.json: {e}")

import base64

def obfuscate_password(plain_password: str) -> str:
    """Simple obfuscation helper to secure password in local JSON files."""
    if not plain_password:
        return ""
    # Avoid double-obfuscating if it's already obfuscated
    if plain_password.startswith("obf:"):
        return plain_password
    key = b"utas_secret_db_key_123!"
    # XOR key cycling
    xor_bytes = bytes([ord(c) ^ key[i % len(key)] for i, c in enumerate(plain_password)])
    return "obf:" + base64.b64encode(xor_bytes).decode("utf-8")

def deobfuscate_password(obfuscated_password: str) -> str:
    """Decodes obfuscated password from local JSON files."""
    if not obfuscated_password:
        return ""
    if not obfuscated_password.startswith("obf:"):
        # Not obfuscated, return plain password (backward compatibility)
        return obfuscated_password
    try:
        raw_base64 = obfuscated_password[4:]
        key = b"utas_secret_db_key_123!"
        decoded_bytes = base64.b64decode(raw_base64.encode("utf-8"))
        xor_chars = [chr(b ^ key[i % len(key)]) for i, b in enumerate(decoded_bytes)]
        return "".join(xor_chars)
    except Exception:
        # Fallback to original value if it can't be decoded
        return obfuscated_password
...
def ensure_tables(connection, db_type, config):
    ...
    try:
        ...
        if db_type == "Oracle":
            # Check if table exists using dummy select
            table_exists = False
            try:
            ...
            if not table_exists:
                ...
                if col_pk:
                    pk_def = f'"{col_pk}" NUMBER PRIMARY KEY'
                    if not seq_name:
                        pk_def = f'"{col_pk}" NUMBER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY'
                    cursor.execute(f"""
            ...
            # Check Attendance Logs Table using dummy select
            table_exists = False
            try:
                cursor.execute(f'SELECT 1 FROM "{table_name}" WHERE 1=0')
                table_exists = True
            ...
            try:
                ...
                if isinstance(configs, list):
                    for config in configs:
                        if isinstance(config, dict) and config.get("database") == selected_db_type:
                            return config
                elif isinstance(configs, dict):
                    if configs.get("database") == selected_db_type:
                        return configs
            except Exception as e:
...
def insert_log_generic(records, machine_ref, company_names: list[str], bypass_offline_save=False):
        ...
        try:
            if db_type == "Oracle":
                ...
                try:
                    ...
                    conn.close()
            success_count += 1
            add_pull_history_entry(machine_ref, company_name, profile_name, len(records), "Success")
        except Exception as e:
            err_msg = f"Database write failed for company '{company_name}' (Profile: '{profile_name}'): {e}"
            logging.error(err_msg)
            errors.append(err_msg)
            add_pull_history_entry(machine_ref, company_name, profile_name, len(records), "Failed")
            
    if errors:
        if not bypass_offline_save:
            try:
                save_offline_logs(records, machine_ref)
            except Exception as save_err:
                logging.error(f"Failed to save offline logs: {save_err}")
...
def insert_log_oracle(connection, attendance_records, machine_info, config):
    ...
    try:
        ...
        # Dynamically inspect actual table columns in Oracle to ensure correct PK fallback
        db_cols = []
        try:
            db_cols = [c.upper() for c in get_table_columns(connection, "Oracle", table)]
        except Exception as col_err:
            logging.error(f"Error fetching columns for PK fallback in Oracle: {col_err}")
        ...
        if not col_pk and table.upper() == "HR_EMP_INOUT_DETAIL":
            if "ID" in db_cols:
                col_pk = "ID"
            elif "HR_ATT_LOG_ID" in db_cols:
                col_pk = "HR_ATT_LOG_ID"
            else:
                col_pk = "HR_ATT_LOG_ID"
                
        if not seq_pk and table.upper() == "HR_EMP_INOUT_DETAIL":
            if col_pk and col_pk.upper() == "HR_ATT_LOG_ID":
...
def load_db_profiles() -> dict:
    ...
    try:
        with open(PROFILES_FILE, 'r', encoding='utf-8') as f:
            ...
            # Deobfuscate passwords for run-time connections
            for name, config in profiles.items():
                if isinstance(config, dict) and "password" in config:
                    config["password"] = deobfuscate_password(config["password"])
            return profiles
    except Exception as e:
...
def save_db_profile(name: str, config: dict) -> bool:
    ...
    try:
        ...
        # Obfuscate all passwords before writing to disk
        import copy
        serialized_profiles = copy.deepcopy(profiles)
        for p_name, p_config in serialized_profiles.items():
            if isinstance(p_config, dict) and "password" in p_config:
                p_config["password"] = obfuscate_password(p_config["password"])
...
def save_config(config: dict) -> bool:
    ...
    try:
        ...
        if "password" in config_copy:
            config_copy["password"] = obfuscate_password(config_copy["password"])
...
OFFLINE_LOGS_FILE = os.path.join(APP_DATA_DIR, "failed_logs.json")

def save_offline_logs(records, machine_ref):
    """Saves logs that failed to insert into the DB to a local file."""
    import datetime
    failed_entries = []
    for r in records:
        ...
        failed_entries.append({
            "user_id": str(uid),
            "timestamp": ts_str,
            "machine": str(machine_ref)
        })
        
    if not failed_entries:
        return
        
    # Read existing failed logs
    existing = []
    if os.path.exists(OFFLINE_LOGS_FILE):
        try:
            ...
            logging.error(f"Error loading failed_logs.json: {e}")
            
    # Add new entries, checking for duplicates
    seen = {f"{e['user_id']}_{e['timestamp']}_{e['machine']}" for e in existing}
    added_count = 0
    for entry in failed_entries:
        key = f"{entry['user_id']}_{entry['timestamp']}_{entry['machine']}"
        if key not in seen:
    ...
    if added_count > 0:
        try:
            ...
            logging.info(f"[OFFLINE LOGS] Saved {added_count} failed logs to {OFFLINE_LOGS_FILE}")
        except Exception as e:
            logging.error(f"Error saving failed_logs.json: {e}")
